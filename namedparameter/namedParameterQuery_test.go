...
func TestQueryParsing(test *testing.T) {
	...
	for _, parsingTest := range queryParsingTests {
		...
		query = NewQuery(parsingTest.Input)
...
func TestStructParameters(test *testing.T) {
	...
	//
	query = NewQuery("SELECT * FROM table WHERE col1 = :Foo AND col2 = :Bar AND col3 = :Baz")
	...
	//
	query = NewQuery("SELECT * FROM table WHERE col1 = :Foo AND col2 = :Bar AND col3 = :Foo AND col4 = :Foo AND col5 = :Baz")
	query.SetValuesFromStruct(singleParam)
